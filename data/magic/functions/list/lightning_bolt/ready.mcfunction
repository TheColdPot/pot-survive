title @s title [{"text":"[","color":"dark_gray"},{"text":"Lightning Bolt","entity":"@s","color":"#F7A5E1","bold":true},{"text":"] ","color":"dark_gray"}]
...
execute unless score @s[tag=magic.Casting] magic.Phase matches 41.. run scoreboard players add @s magic.Phase 1

execute if entity @s[tag=magic.Casting,scores={staff.rightClick=1..}] run scoreboard players reset @s magic.Cast

execute if score @s[tag=magic.Casting] magic.Cast matches 5.. run function magic:list/lightning_bolt/summon
...
execute if score @s[tag=magic.Casting] magic.Phase matches 41.. run title @s subtitle [{"text":"● ● ","color":"gray"}]
